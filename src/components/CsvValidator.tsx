...
export default function CsvValidator({ mealPlan }: CsvValidatorProps) {
  // Main Database State
  ...
  // Auto-run validation whenever CSV data changes to keep it synchronized
  useEffect(() => {
    if (csvDataMain.length > 0 || csvDataSec.length > 0) {
      generateReport();
    }
  }, [csvDataMain, csvDataSec]);
